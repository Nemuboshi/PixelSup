...
// RootCommand manages top-level dispatch for parser/ocr subcommands.
...
func (r *RootCommand) Commands() []*Command {
	...
	if parser, ok := r.commands["parser"]; ok {
...
func runParserCommand(args []string, out io.Writer) error {
	...
	if err != nil {
		return fmt.Errorf("compose sheets: %w", err)
	}
	progressDone(out)
...
func runOCRCommand(args []string, out io.Writer) error {
	...
	outPath, err := runOCROnOutput(outputDir, configPath, !writeNew, func(done, total int, _ string) {
...
func writeOCRUsage(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage:")
	_, _ = fmt.Fprintln(w, "  pixelsup-go ocr <output_dir> [--config <yaml>] [--write-new]")
	_, _ = fmt.Fprintln(w, "")
	_, _ = fmt.Fprintln(w, "Options:")
	_, _ = fmt.Fprintln(w, "  --config <yaml>  OCR config file path. Default: ./ocr_config.yaml")
	_, _ = fmt.Fprintln(w, "  --write-new      Write timeline.ocr.srt instead of overwriting timeline.srt")
...
// parseParserArgs validates parser flags and returns parsed options.
func parseParserArgs(args []string) (parserOptions, error) {
	opts := parserOptions{gap: 12, limit: 40, maxWidth: 1080, padding: 10, showRowIdx: true}
	...
	for i := 0; i < len(args); i++ {
		...
		case "-o", "--output":
			if i+1 >= len(args) {
				return opts, fmt.Errorf("failed to parse parser flags: flag needs an argument: %s", arg)
			}
			opts.output = args[i+1]
			i++
		case "--gap":
			if i+1 >= len(args) {
				return opts, errors.New("failed to parse parser flags: flag needs an argument: --gap")
			}
			v, err := strconv.Atoi(args[i+1])
			if err != nil {
				return opts, errors.New("--gap must be an integer")
			}
			opts.gap = v
			i++
		case "--limit":
			if i+1 >= len(args) {
				return opts, errors.New("failed to parse parser flags: flag needs an argument: --limit")
			}
			v, err := strconv.Atoi(args[i+1])
			if err != nil {
		...
		case "--max-width":
			if i+1 >= len(args) {
				return opts, errors.New("failed to parse parser flags: flag needs an argument: --max-width")
			}
			v, err := strconv.Atoi(args[i+1])
			if err != nil {
		...
		case "--padding":
			if i+1 >= len(args) {
				return opts, errors.New("failed to parse parser flags: flag needs an argument: --padding")
			}
			v, err := strconv.Atoi(args[i+1])
			if err != nil {
		...
		case "--no-row-index":
			opts.showRowIdx = false
		default:
			if strings.HasPrefix(arg, "-") {
				return opts, fmt.Errorf("failed to parse parser flags: unknown flag: %s", arg)
			}
	...
	if len(inputs) != 1 {
		return opts, errors.New("parser requires exactly one input path")
	}
	opts.input = inputs[0]
	return opts, nil
}
...
func validateParserOptions(opts parserOptions) error {
	...
	if opts.gap < 0 {
		return errors.New("--gap must be >= 0")
	}
	if opts.maxWidth <= 0 {
...
// writeParserUsage documents parser invocation and supported flags.
func writeParserUsage(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage:")
	_, _ = fmt.Fprintln(w, "  pixelsup-go parser <input.sup|input.idx|image_dir> [-o <outdir>] [--limit N] [--gap N] [--max-width N] [--padding N] [--force-white] [--no-row-index]")
	_, _ = fmt.Fprintln(w, "")
	_, _ = fmt.Fprintln(w, "Options:")
	_, _ = fmt.Fprintln(w, "  -o, --output <outdir>  Output directory. Default derived from input path")
	_, _ = fmt.Fprintln(w, "  --gap <n>               White gap between rows. Default: 12")
	_, _ = fmt.Fprintln(w, "  --limit <n>             Max cues per sheet. Default: 40")
	...
	_, _ = fmt.Fprintln(w, "  --no-row-index          Disable left row index labels")
}
